"""
Train AETHER model on SAP BSP669 Enterprise ERP dataset.

Uses the same architecture as Sepsis/BPI models:
  EventEncoder -> TransitionModel -> EnergyScorer -> HierarchicalPredictor

Note: This dataset has 77 activities - larger vocabulary than other datasets,
representing complex O2C and P2P ERP transaction flows.
"""
...
def main():
    ...
    logger.info("SAP BSP669 Enterprise ERP Dataset - 77 activities (larger vocabulary)")
    ...
    logger.info(f"Activity vocab: {activity_vocab.size} tokens (77 unique activities)")
    logger.info(f"Resource vocab: {resource_vocab.size} tokens")

    # Create datasets
    # Note: max_seq_len=100 to handle longer ERP transaction sequences
    ...
    # Note: n_phases=6 for more complex ERP process phases
    ...
    # Print final summary
    logger.info("=" * 60)
    logger.info("TRAINING COMPLETE - SAP BSP669 Enterprise ERP")
