"""
Benchmark AETHER governance formula on Judicial Case Events data.

FIRST legal process mining domain with uncertainty quantification.
...
DATA_DIR = AETHER_ROOT / "data" / "external" / "judicial"
...
def load_model(vocab_path, model_path, device):
    ...
    predictor = HierarchicalPredictor(latent_dim=128, n_activities=activity_vocab.size, n_phases=3)
...
def main():
    print("=" * 60)
    print("JUDICIAL CASE EVENTS GOVERNANCE BENCHMARK")
    print("FIRST Legal Process Mining with Uncertainty Quantification")
    print("=" * 60)
    ...
    for case in cases:
        ...
        results.append({
            ...
            "doctrine": case.get("caseAttributes", {}).get("primary_doctrine", "unknown"),
        })
    ...
    # Analyze by doctrine
    doctrine_stats = defaultdict(lambda: {"correct": 0, "wrong": 0})
    for r in results:
        doctrine = r.get("doctrine", "unknown")
        if r["is_correct"]:
            doctrine_stats[doctrine]["correct"] += 1
        else:
    ...
    for doctrine, stats in sorted(doctrine_stats.items()):
        ...
        print(f"  {doctrine}: {acc:.1%} ({stats['correct']}/{total})")

    # Benchmark each mode
    benchmark_results = {
        "benchmark": "AETHER Judicial Case Events Governance Benchmark (v2 bidirectional)",
        "novelty": "FIRST legal process mining domain with uncertainty quantification",
        "dataset": {
            "label": "Judicial Case Events",
            ...
            "doctrines": list(doctrine_stats.keys()),
        },
        "model": {
    ...
    print(f"  Effective threshold: {benchmark_results['per_mode']['standard']['aether']['threshold_mean']:.4f}")

    if std_factors['unc_total'] > 1.0:
        print("\n  [ADAPTED] Higher uncertainty in legal domain -> raised threshold for more review")
    else:
        print("\n  [ADAPTED] Lower uncertainty -> standard threshold applies")
