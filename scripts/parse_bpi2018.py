"""
Parse BPI Challenge 2018 Agriculture XES file into AETHER event log format.

Dataset: BPI Challenge 2018 - German Agriculture Subsidy Applications
- 43,809 cases
- 2,514,266 events
- ~14 unique activities (aggregated from doctype/subprocess/activity)
- Process: EU Common Agricultural Policy subsidy distribution

Due to the massive size (2.5M events), this parser supports:
- Limiting to first N cases for initial training
- Memory-efficient iterparse processing

Source: https://data.4tu.nl/articles/dataset/BPI_Challenge_2018/12688355
"""
...
XES_PATH = DATA_DIR / "BPI Challenge 2018.xes"
OUTPUT_DIR = DATA_DIR

# Configuration for subset processing
MAX_CASES = 10000  # Use first 10K cases for training (manageable size)
...
def parse_timestamp(ts_str: str) -> Optional[datetime]:
    ...
    if not ts_str:
        return None
    try:
        # XES timestamps: 2015-05-08T00:00:00.000+02:00
...
def parse_xes_file(xes_path: Path, max_cases: Optional[int] = None) -> list[dict]:
    ...
    Uses iterparse for memory efficiency on the 1.9GB file.
    """
    ...
    for event_type, elem in context:
            ...
            if tag == "trace":
                if current_events:
                    ...
                    if events:
                        ...
                        # Determine outcomes based on trace attributes and activities
                        activities = [e["activity"] for e in events]

                        # Payment and subsidy outcomes
                        has_payment = any("payment" in a.lower() for a in activities)
                        is_rejected = trace_attributes.get("rejected", False)
                        payment_actual = trace_attributes.get("payment_actual0", 0.0)
                        amount_applied = trace_attributes.get("amount_applied0", 0.0)

                        # Compute if on-time (payment made without rejection)
                        on_time = has_payment and not is_rejected

                        # Rework = multiple edits or resubmissions
                        edit_count = sum(1 for a in activities if "edit" in a.lower())
                        has_rework = edit_count > 2

                        case = {
                            "caseId": case_id,
                            "events": events,
                            "source": "bpi_challenge_2018",
                            "processType": "agriculture_subsidy",
                            "outcome": {
                                "paid": has_payment,
                                "rejected": is_rejected,
                                "complete": has_payment or is_rejected,
                                "durationHours": round(duration_hours, 2),
                                "paymentActual": payment_actual,
                            ...
                            "caseAttributes": {
                                ...
                                if k not in ("concept:name", "identity:id")
                                and not k.startswith("penalty_")  # Skip many penalty flags
                            },
                        }
                        cases.append(case)
                        case_count += 1
                ...
                for child in elem:
                    ...
                    if key == "concept:name":
                        ...
                        # BPI 2018 has both concept:name and activity
                        # Prefer activity if more specific
                        if not event_data.get("activity") or event_data["activity"] == "":
                            event_data["activity"] = value
                        ...
                        event_attrs["doctype"] = value
                    elif key == "subprocess":
                        event_attrs["subprocess"] = value
                    elif key == "success":
                        event_attrs["success"] = value.lower() == "true" if value else False
                    elif key not in ("identity:id", "docid", "eventid", "note", "docid_uuid"):
                ...
                # Create composite activity if needed (BPI 2018 has rich activity structure)
                if not event_data.get("activity"):
                    # Compose from doctype + subprocess + activity if available
                    doctype = event_attrs.get("doctype", "")
                    subprocess = event_attrs.get("subprocess", "")
                    activity = event_attrs.get("activity", "")
                    if doctype or subprocess or activity:
                        parts = [p for p in [doctype, subprocess, activity] if p]
                        event_data["activity"] = " - ".join(parts) if parts else "unknown"
                    else:
                        event_data["activity"] = "unknown"
        ...
        # Check if we should stop (after max_cases)
        if max_cases and case_count >= max_cases:
            break
...
def compute_stats(cases: list[dict]) -> dict:
    ...
    rejected_count = sum(1 for c in cases if c["outcome"]["rejected"])
    ...
    return {
        ...
        "activity_counts": dict(activities.most_common(30)),  # Top 30
        "outcome_stats": {
            "paid_rate": round(paid_count / len(cases), 4) if cases else 0,
            "rejected_rate": round(rejected_count / len(cases), 4) if cases else 0,
            "complete_rate": round(complete_count / len(cases), 4) if cases else 0,
            "rework_rate": round(rework_count / len(cases), 4) if cases else 0,
        },
        "event_length_stats": {
...
def main():
    print("=" * 60)
    print("BPI 2018 AGRICULTURE -> AETHER EVENT LOG PARSER")
    print("=" * 60)
    ...
    # Parse XES file (with case limit for manageable training)
    cases = parse_xes_file(XES_PATH, max_cases=MAX_CASES)
    ...
    stats["max_cases_limit"] = MAX_CASES
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("BPI 2018 AGRICULTURE DATASET SUMMARY")
    print("=" * 60)
    ...
    print(f"  Rework: {stats['outcome_stats']['rework_rate']:.1%}")
