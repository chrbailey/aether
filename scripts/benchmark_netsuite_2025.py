"""
Benchmark AETHER governance formula on NetSuite 2025 Transaction data.

This is the largest real transaction dataset (4.2M source rows).
...
DATA_DIR = AETHER_ROOT / "data" / "external" / "netsuite_2025"
...
def main():
    print("=" * 60)
    print("NETSUITE 2025 GOVERNANCE BENCHMARK")
    print("Largest real transaction dataset (4.2M source rows)")
    print("=" * 60)
    ...
    # Benchmark each mode
    benchmark_results = {
        "benchmark": "AETHER NetSuite 2025 Governance Benchmark (v2 bidirectional)",
        "dataset": {
            "label": "NetSuite 2025",
    ...
    benchmarks_path = BENCHMARKS_DIR / "netsuite_2025.json"
    shutil.copy(output_path, benchmarks_path)
    print(f"Copied to: {benchmarks_path}")
    ...
    print(f"Model Accuracy: {accuracy:.1%}")
    print(f"Model ECE: {ece:.4f}")
    standard_mode = benchmark_results["per_mode"]["standard"]
    print(f"MCC Improvement (standard mode): {standard_mode['mcc_improvement']:+.4f}")
    print(f"  Static MCC:  {standard_mode['static']['mcc']:.4f}")
    print(f"  AETHER MCC:  {standard_mode['aether']['mcc']:.4f}")
