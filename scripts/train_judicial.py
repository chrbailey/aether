"""
Train AETHER model on Judicial Case Events dataset.

FIRST legal process mining domain with uncertainty quantification.
Uses same architecture as other AETHER models:
  EventEncoder -> TransitionModel -> EnergyScorer -> HierarchicalPredictor

Dataset: Judge Alsup N.D. Cal rulings (2010-2025)
Cases: 21 total (16 train, 5 val)
Events: Motions, rulings, settlements
"""
...
LATENT_DIM = 128
N_EPOCHS = 50
BATCH_SIZE = 8  # Smaller batch size for 21 cases
LR = 3e-4
...
def main():
    ...
    logger.info("Domain: Legal Process Mining (NOVEL)")
    ...
    train_dataset = EventSequenceDataset(
        ...
        max_seq_len=16,  # Short sequences (max 5 events in dataset)
    ...
    predictor = HierarchicalPredictor(
        ...
        n_phases=3,  # Fewer phases for legal process (filing -> motion -> resolution)
    )
    ...
    # Print final summary
    logger.info("=" * 60)
    logger.info("TRAINING COMPLETE - Legal Process Mining Model")
